...
public class Secret {
    ...
    public static class AES {
        ...
        public static String decrypt(String key,String iv,byte[] data){
            ...
            /*String temp = parseByte2HexStr(key.getBytes());
            key = temp.substring(0, 16);*/
            byte[] enCodeFormat = key.getBytes();
            SecretKeySpec secretKeySpec = new SecretKeySpec(enCodeFormat, "AES");
...
//                cipher.init(Cipher.DECRYPT_MODE, secretKeySpec, new IvParameterSpec(null));// 初始化
                cipher.init(Cipher.DECRYPT_MODE, secretKeySpec);
                byte[] result = cipher.doFinal(data);
                content = new String(result);
            } catch (NoSuchAlgorithmException e) {
                Log.e("AESS", e.getCause().getMessage());
                e.printStackTrace();
            } catch (NoSuchPaddingException e) {
                WLibLog.e(e);
                e.printStackTrace();
        ...
        private static final String DEFAULT_HOST_KEY = "736d14293a3a7737";
        /**
         * @param data 需要解密的数据
         * @return
         */
        public static String decrypt(byte[] data){
            String content = "";
            byte[] enCodeFormat = DEFAULT_HOST_KEY.getBytes();
            SecretKeySpec secretKeySpec = new SecretKeySpec(enCodeFormat, "AES");
            Cipher cipher = null;// 创建密码器
            try {
                cipher = Cipher.getInstance(algorithmStr);
...
//                Log.i("Aes","hex=>>>>>>>>>>>"+hex.toUpperCase());
        ...
        private static final String keyBytes = "video123";
//                "a2cdefgabcdefg15";
